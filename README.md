...
| **Web dashboard** | A read-only, single-file HTML page: today's list and the goal cards up front, everything else one click away, attention and advice in the sidebar. Tick items off as you go, pick an option on an advice card, and copy the result back to Mishu. Light and dark themes, works on mobile. |
