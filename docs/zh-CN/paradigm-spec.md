...
网页看板 `dashboard.html` 只读展示同一份数据，但刻意更安静：主栏放**今日主线和目标卡片**，侧栏放**需要关注、秘书建议、需要补充、已完成**。目标卡片只留标题、进度、节奏/截止/本周、下一步；阶段、编号行动清单、近 14 天投入图、日志和决策都收在「详情」里。今日清单只显示任务标题，展开才看到所属目标、编号和备注。习惯、事务和晚间回顾仍然写进 Markdown 每日清单，但不在网页看板上显示；回顾过的事项只在标题旁标出结果。数据为空时显示引导面板。

行动和今日清单都带**复选框**。勾选不会改动档案库：状态只存在浏览器的 localStorage，右下角的小浮窗提供**「一键回复秘书」**，把勾选结果复制成一段现成的话（「秘书，今天（日期）的情况：✓ G02.T03 …」），粘回对话即可。档案库仍然只由 `mishu.py` 写入。
