...
| **网页看板** | 只读的单文件 HTML：主栏是今日清单和目标卡片，点卡片看细节，需要关注和秘书建议在侧栏；事项可以勾选、建议可以选项，再一键复制给秘书。支持浅色 / 深色主题，手机上也能看 |
| **已完成回溯** | 完成的目标自动归档并记下完成日期；网页看板列出每个已完成目标的简介（起止日期、用时天数、累计投入、完成标准），也可以用 `done` 回溯全部 |
...
| "我都完成过哪些目标？" | 列出已完成的目标，附起止日期、用时和投入 |
